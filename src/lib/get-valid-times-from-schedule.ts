...
export async function getValidTimesFromSchedule(
   ...
   const groupedAvailabilities = groupBy(schedule.availabilities, 'dayOfWeek');
   ...
   return timesInOrder.filter((intervalDate) => {
      ...
      return (
         ...
         availabilities.some((availability) => {
            return (
               isWithinInterval(eventInterval.start, availability) && isWithinInterval(eventInterval.end, availability)
            );
         })
      );
   });
...
function getAvailabilities(
   groupedAvailabilities: Partial<
      Record<(typeof DAYS_OF_WEEK_IN_ORDER)[number], (typeof ScheduleAvailabilityTable.$inferSelect)[]>
   >,
   date: Date,
   timezone: string
) {
...
function groupBy<T extends keyof Availabilities>(array: Availabilities[], key: T) {
   return array.reduce(
      (result: Record<Availabilities[T], Availabilities[]>, currentValue) => {
      ...
      {} as Record<Availabilities[T], Availabilities[]>
   );
}
